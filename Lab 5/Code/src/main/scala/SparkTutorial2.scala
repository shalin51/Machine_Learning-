     ...
     val lemmatisedQuestion = new ListBuffer[String]()
      ...
      if (QuestionAns.GotTheAnswer(FinalAnswer)) {
        ...
        if (topic.nonEmpty) {
          ...
          if (QuestionAns.GotTheAnswer(FinalAnswer)) {
          ...
          else {TF_IDF.FindSynonyms(GetMainPhrase(),sContext)
            ...
            val FinalAnswer = QuestionAns.GetFinalAns(questionType, ans, formattedQuestion)
            if (QuestionAns.GotTheAnswer(FinalAnswer)) {
              val topic= LDA.FindoutTopic()
              val getDocuments=KMeansOperationGetCluster(topic)
              val ans=KMeansOperation.GetAnswreFromCluster(getDocuments)
              println(QuestionAns.GetFinalAns(questionType,ans,formattedQuestion))
            }
          }
        }
        else {
